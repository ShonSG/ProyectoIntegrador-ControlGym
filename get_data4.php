...
if(isset($_POST['custId'])){
	...
	$sql = "select * from instructor WHERE dni=".$id;
	$result = mysqli_query($con,$sql);

$response = " <form role='form' method='post' name='editar-instructor' id='editar-instructor' action='insertar-admin.php'>
<div class='box-body'>";
while( $row = mysqli_fetch_array($result) ){
	...
	<input type='text' class='form-control formulario__input' id='DNI' name='DNI' placeholder='DNI' value='$dnic' readonly>
  ...
  $response .="</div>";
  $response .="
<input type='hidden' name='editar-instructor' value='1'>
<button type='submit' class='btn btn-primary'>Editar</button>";
}

$response.="</form>";
$response.="<script src='js/admin-ajax.js'></script>
